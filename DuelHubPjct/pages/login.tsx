...
const Login: NextPageWithProps = () => {
  ...
  const buttons: { text: string; type: LoginType; icon: IconName }[] = [
    { text: "Enter with Google", type: LoginType.GOOGLE, icon: "google" },
    { text: "Enter with GitHub", type: LoginType.GITHUB, icon: "github" },
    { text: "Enter with LinkedIn", type: LoginType.LINKEDIN, icon: "linkedin" },
    { text: "Enter with Discord", type: LoginType.DISCORD, icon: "discord" },
  ];
...
Login.getLayout = (page) => (
  <div className="h-screen bg-[#252558]">
    ...
    <footer className="p-3 fixed bottom-0 w-full text-center bg-[#679BF9]">
      &copy;{new Date().getFullYear()} DuelHub
    </footer>
  </div>
);
